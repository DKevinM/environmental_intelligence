...
def main():
 ...
 try:
  w=load_weather(cfg); aq=load_current_aqhi(cfg); fx=load_forecast_aqhi(cfg); aq['blend']=load_blend_estimate(cfg); aq['pollutant']=load_nearest_pollutant(cfg); aq['purpleair']=load_nearest_purpleair(cfg); a=assess(cfg,w,aq,fx); fire=load_hotspots(cfg); cams=load_nearby_cameras(cfg); traj=load_trajectory(cfg); n=build(cfg,w,aq,fx,a,fire,traj); mp=map_layers.build(cfg); mp['fire']=fire; mp['trajectory']=traj; now=datetime.now(ZoneInfo(cfg['project']['timezone'])).isoformat(timespec='seconds'); p={'generated_at':now,'event':cfg['event'],'weather':w,'air_quality':{'current':aq,'forecast':fx},'assessment':a,'narrative':n,'map':mp,'cameras':cams};write_json(out/'dashboard_data.json',p);write_json(out/'intelligence_summary.json',{'generated_at':now,'assessment':a,'narrative':n});(out/'dashboard.html').write_text(build_html(cfg,p));print(f"Overall risk: {a['overall_risk']}\nDashboard: {out/'dashboard.html'}");return 0
